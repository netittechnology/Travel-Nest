...
import { Search, Plus, Pencil, Trash2, Eye, EyeOff, ChevronLeft, ChevronRight, Sparkles } from "lucide-react";
...
export default function ExperienceList() {
  ...
  const handleDelete = async (id) => {
    const ok = await confirmAction({ title: "Delete experience?" });
    if (!ok) return;
  ...
  const handleToggle = async (id) => {
    await axiosInstance.put(`/experiences/${id}/toggle-availability`);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-500">
                Create, manage, publish and organize your travel experiences.
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-400">
                  Manage your published and unpublished experiences.
                </p>
              </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? (
                  filteredData.map((item) => (
                    <tr key={item.id} className="group transition-colors duration-150 hover:bg-[#172d35]/[0.03]">
                      ...
                      {/* STATUS */}
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${item.is_available ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" : "bg-slate-100 text-slate-600 ring-1 ring-slate-200"}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${item.is_available ? "bg-emerald-500" : "bg-slate-400"}`} />
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {/* EDIT */}
                          <button
                            onClick={() => navigate(`/admin/experiences/edit/${item.id}`)}
                          ...
                          <button
                            ...
                            title={item.is_available ? "Unpublish experience" : "Publish experience"}
                            className={`inline-flex h-9 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold text-white transition-all ${item.is_available ? "bg-amber-500 hover:bg-amber-600" : "bg-emerald-600 hover:bg-emerald-700"}`}
                          >
                    ...
                    <td colSpan="5" className="px-5 py-16 text-center">
                      <div className="mx-auto flex max-w-sm flex-col items-center">
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          Try changing your search or add a new experience.
                        </p>
                      </div>
                    </td>
                  </tr>
